...
internal static class AnalysisIntrospectionSupport
{
    ...
    {
        var processResult = await AnalysisRuntimeSupport.InvokeProcessCaptureAsync(
            commandPath,
            argumentList,
            workingDirectory,
            environment,
            timeoutSeconds,
            cancellationToken);
        var preferredMessage = AnalysisRuntimeSupport.GetPreferredMessage(processResult.Stdout, processResult.Stderr);
        var classification = AnalysisIntrospectionFailureClassifier.Classify(argumentList, preferredMessage);
        var parse = AnalysisIntrospectionPayloadParser.TryParse(expectedFormat, processResult.Stdout);
