internal static class ToolHelpRootCommandInventoryInference
{
    ...
    {
        var aliasInventoryLines = ToolHelpRootCommandAliasInventorySupport.InferAliasInventoryLines(preamble.Skip(1).ToArray());
    ...
    public static bool LooksLikeAliasCommandInventoryBlock(IReadOnlyList<string> lines)
        => ToolHelpRootCommandAliasInventorySupport.InferAliasInventoryLines(lines).Count > 0;
